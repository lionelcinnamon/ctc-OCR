...
from ..utility.md_utils import performance_time
...
class Img2SeqCtcModel(BaseModel):
    ...
    def __init__(self, config):
        ...
        """
        super(Img2SeqCtcModel, self).__init__(config)
        self._vocab = Vocab(config)

    def build_train(self):
        """Builds model"""
        self.logger.info("Building model...")
        # self.seq_len = [self.config.max_length_formula] * self.config.batch_size
        self.encoder = Encoder(self.config)
        self.decoder = Decoder(self.config, self._vocab.n_tok)

        self._add_placeholders_op()
        self._add_pred_op()
        self._add_loss_op()

        self._add_train_op(self.config.lr_method, self.lr, self.loss, self.config.clip)
        self.init_session()
    ...
    def _add_placeholders_op(self):
        ...
        # input of the graph
        self.img = tf.placeholder(tf.uint8, shape=(None, 48, None, 1), name='img')
    ...
    def _run_epoch(self, config, train_set, val_set, epoch, lr_schedule):
        ...
        for i, (img, formula) in enumerate(minibatches(train_set, batch_size)):
            ...
            prog.update(i + 1, [("loss", loss_eval), ("acc", acc), ("lr", lr_schedule.lr)])
        ...
        # evaluation
        config_eval = Config({
            "dir_answers": config.get_file_path(config.dir_output, "formulas_val"),
            "batch_size": config.batch_size
        })
    ...
    def _run_evaluate(self, config, test_set):
        ...
        for i, (img, formula) in enumerate(minibatches(test_set, batch_size)):
            ...
            loss_eval, acc = self.sess.run([self.loss, self.predict_accuracy], feed_dict=fd)
            prog.update(i + 1, [("loss", loss_eval), ("acc", acc)])
    ...
    def predict_batch(self, images):
        ...
        for i, pred in enumerate(preds):
            ...
            # print(p)
            # print(self._vocab)
            # print(self._vocab.id_to_tok)
            p = "".join([self._vocab.id_to_tok[idx] for idx in p if idx != -1])
            hyps.append(p)
